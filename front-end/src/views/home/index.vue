...
export default {
  ...
  methods: {
    ...
    async searchCompany() {
      ...
      for (let i = 0; i < resultCompany.length; i++) {
        ...
        //   "data:image/jpeg;base64," + resultCompany[i]._source.image;
         resultCompany[i]._source.image = process.env.VUE_APP_SERVER + resultCompany[i]._source.image;
    ...
    async getListCompany(paging) {
      ...
      this.itemsCompanyList.length > 0
        ? (this.overlay = false)
        : (this.overlay = true);
    },
    async getCompanyAndAddress() {
      ...
      for (let i = 0; i < resultCompanyForselect.length; i++) {
        resultCompanyForselect[i].companyNm = resultCompanyForselect[
          i
        ].companyNm.concat(" - " + resultCompanyForselect[i].addressCd);
      }
